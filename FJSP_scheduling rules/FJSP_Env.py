    ...
    def step(self, action,mch_a):
        ...
        for i in range(self.batch_sie):
            ...
            if action[i] not in self.partial_sol_sequeence[i]:
                ...
                self.dur_a = int(self.dur[i,row, col,mch_a[i]]) #加工时间
                ...
                if action[i] not in self.last_col[i]:
                    ...
                    self.mask[i,row] = 1
                #print(self.dur_a)
                self.temp1[i,row, col] = startTime_a + self.dur_a #工序的完工时间 批次*工件数*最大工序数
                #print(self.temp1)
...
#--------------------------------------------------------------------------------------------------------------------
    ...
    def reset(self, data,rule):
        ...
        self.dur = data.astype(float)#single单精度浮点数
        ...
        for t in range(self.batch_sie):
            ...
            for i in range(self.number_of_jobs):
                ...
                for j in range(self.max_operation):
                    ...
                    self.mask_mch[t][i][j] = [1 if k <= 0 else 0 for k in self.dur_cp[t][i][j]]
                    self.dur[t][i][j] = [100 if k <= 0 else k for k in self.dur[t][i][j]]
                    if len(durmch) == 0:
                        dur_min.append(1)
                        dur_mean.append(1)
                        dur_max.append(1)
...
#--------------------------------------------------------------------------------------------------------------------------
        ...
        self.omega = self.first_col.astype(np.int64) #每个工件下一个要调度的工序 索引 （已加工完成的为 最后一个工序）
