...
class MemTracker(object):
    ...
    def get_tensor_usage(self):
        sizes = [np.prod(np.array(tensor.size())) * get_mem_space(tensor.dtype) for tensor in get_tensors()]
        ...
        with open(gpu_profile_fn, 'a+') as f:
            ...
            if begin:
                f.write(f"GPU Memory Track | {datetime.datetime.now():%d-%b-%y-%H:%M:%S} |"
                        f" Total Tensor Used Memory:{get_tensor_usage():<7.1f}Mb"
                        f" Total Allocated Memory:{get_allocate_usage():<7.1f}Mb\n\n")
                begin = False

            if print_detail is True:
                ts_list = [(tensor.size(), tensor.dtype) for tensor in get_tensors()]
                new_tensor_sizes = {(type(x),
                                    tuple(x.size()),
                                    ts_list.count((x.size(), x.dtype)),
                                    np.prod(np.array(x.size()))*get_mem_space(x.dtype)/1024**2,
                                    x.dtype) for x in get_tensors()}
                for t, s, n, m, data_type in new_tensor_sizes - last_tensor_sizes:
                    f.write(f'+ | {str(n)} * Size:{str(s):<20} | Memory: {str(m*n)[:6]} M | {str(t):<20} | {data_type}\n')
                for t, s, n, m, data_type in last_tensor_sizes - new_tensor_sizes:
                    f.write(f'- | {str(n)} * Size:{str(s):<20} | Memory: {str(m*n)[:6]} M | {str(t):<20} | {data_type}\n')
