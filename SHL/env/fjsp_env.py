...
class FJSPEnv(gym.Env):
    ...
    def __init__(self, case, env_paras, data_source='case'):
        ...
        self.ope_ope_adj_batch = torch.stack(tensors[8], dim=0).long()  # shape: (batch_size, num_opes, num_opes)
        ...
        self.feat_opes_batch = feat_opes_batch
        self.feat_mas_batch = feat_mas_batch
        self.adap_feat_opes_batch = feat_opes_batch[:,[0,1, 2,4, 5],:]   # shape: (batch_size, 3, num_opes)
        self.adap_feat_mas_batch =  feat_mas_batch[:,[0, 2],:]  # shape: (batch_size, 2, num_mas)
    ...
    def step(self, actions):
        ...
        self.mask_job_procing_batch[
            self.batch_idxes, jobs] = True  # True for jobs in process shape: (batch_size, num_jobs),
        self.mask_ma_procing_batch[
            self.batch_idxes, mas] = True  # shape: (batch_size, num_mas), True for machines in process
        self.mask_job_finish_batch = torch.where(
            self.ope_step_batch == self.end_ope_biases_batch + 1,
            True,
            self.mask_job_finish_batch)  # shape: (batch_size, num_jobs), True for completed jobs
        ...
        info = []
        self.adap_feat_opes_batch = self.feat_opes_batch[:, [0,1, 2,4, 5],:]  # shape: (batch_size, 3, num_opes)
        self.adap_feat_mas_batch = self.feat_mas_batch[:, [0, 2],:]  # shape: (batch_size, 2, num_mas)
